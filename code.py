...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    result = df[(df[col1]>val1)&(df[col2]==val2)]
    return result
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    return pd.pivot_table(df, index= datetime.dt.month, values= agg_col , aggfunc = agg)
...
val_count = clear(weather, 'Weather', 'Cloudy')
#print(val_count)
...
mean_temp = agg_values_ina_month(weather, 'Date/Time', 'Temp (C)', 'mean')
#print(mean_temp)
...
fahrenheit_temp = convert(weather, 'Temp (C)')
fahrenheit_dew_point_temp = convert(weather, 'Dew Point Temp (C)')
